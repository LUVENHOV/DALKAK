...
async def prefer_recommend(m_id: int, session: Session = Depends(engineconn.get_session)):
    try:
...
async def refrigerator_recommend(m_id:int, session: Session = Depends(engineconn.get_session)):
    try:
